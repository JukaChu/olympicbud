...
*/	
	...
	if(isset($_REQUEST['yourPhone'])){
		...
		$nertype = strip_tags(trim($_REQUEST['nertype']));
		$send_result_polzunok = strip_tags(trim($_REQUEST['send-result-polzunok']));
		$construction = strip_tags(trim($_REQUEST['construction']));
		$proekt = strip_tags(trim($_REQUEST['proekt']));
		$techbud = strip_tags(trim($_REQUEST['techbud']));
		$orderRoof33 = strip_tags(trim($_REQUEST['orderRoof33']));
		...
		$body = '<table>
					...
					<tr><td>Этажность:</td><td>'.$nertype.'</td></tr>
					<tr><td>Площадь:</td><td>'.$send_result_polzunok.'</td></tr>
					<tr><td>Земля есть?:</td><td>'.$construction.'</td></tr>
					<tr><td>Проект есть?:</td><td>'.$proekt.'</td></tr>
					<tr><td>Материал:</td><td>'.$techbud.'</td></tr>
					<tr><td>Когда начало?:</td><td>'.$orderRoof33.'</td></tr>
				</table>';
